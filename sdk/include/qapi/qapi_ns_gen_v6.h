 ...
 * The link local address format is fe80::/64. The
 ...
 *
 * @return    1 if the IPv6 address is link local, 0 otherwise.
 * @hideinitializer
 */
...
} /** @cond */qapi_Net_Ping_V6_t/** @endcond */;
...
typedef struct
{
    ...
    /**< Number of #qapi_Net_IPv6_Route_t arrays in the routing table. */
 ...
 * @param[in] interface_Name  Pointer to the interface name; the interface name is 
...
/**	@ingroup qapi_Net_Ping6_2
 * @brief Sends an IPv6 ping request with a response.
 *
 * @param[in]      ping6_buf    Pointer to the IPv6 ping structure. The structure will
                                take the IPv6 address to which to send a ping, the IPv6 address 
                                to send the ping via this source, the number of data bytes
                                to send, the ping request timeout value (in ms), and 
                                when pinging an IPv6 link local address interface, a 
                                name is required.
 * @param[out]     ping_resp    Pointer to where to store the ping response code and
 *                              the type for the ICMP echo response received.
 *
 * @return
